...
class Wind:
    ...
    def calc_init_grid(self):
        ...
        if self.steady_state_on:
            ...
            (v,w) = (self.K_steady_state/10)*self.steady_state_max*math.Vector2(random.uniform(-1, 1), random.uniform(-1, 1))
            self.wind_field[:,:,0] = v
            self.wind_field[:,:,1] = w
    ...
    def new_gust(self):
        ...
        lg = random.loguniform(0.1,2)
        if (self.t==0):
            t0 = random.loguniform(-lg,0)    #offset for how far along in time the gust is
        else:
            t0 = self.t
        ...
        if self.gusts_on:
            ...
            for i in range(len(self.gust_params)):
                ...
                if (rel_t<self.gust_params[i][3]):
                    V = (self.gust_params[i][2]/2.0)*(1-np.cos((2*np.pi*self.t)/self.gust_params[i][3]))
    ...
    def get_wind(self,dt,position_px):        
        ...
        return math.Vector2(self.wind_field[position_px[0],position_px[1],0],self.wind_field[position_px[0],position_px[1],1])
